  ...
  public render() {
    ...
    const linkList = (status.code !== 5 ? (statusList.length && statusList.slice(0, 5).map((item, key) => {
      ...
      return {
        ...
        logTime: current ? new Date(+new Date(new Date(current.logTime)
                .toJSON()) + 8 * 3600 * 1000)
                .toISOString()
                .replace(/T/g, ' ')
                .replace(/\.[\d]{3}Z/, '') : null,
      ...
      return {
        status: intl.locale === 'zh' ? 
          statusList[item.currStatus].label + '环节' : 
          statusList[item.currStatus].english,
        logTime: new Date(+new Date(new Date(item.logTime)
          .toJSON()) + 8 * 3600 * 1000)
          .toISOString()
          .replace(/T/g, ' ')
          .replace(/\.[\d]{3}Z/, ''),
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    {
                      forwardLog.map((item)=> (
                        item.fwType === 0 && <div className={styles.circle} key={item.fwResult}>
                          <span style={{ background: item.fwResult === 'SUCCESS' ? '#02B583' :  '#F54238'}} />
                          {formatMessage(item.fwResult === 'SUCCESS' ? search_msg.event_success : search_msg.event_fail)}
                        </div>
                      ))
                    }
                  </td>
                ...
                <tr>
                  ...
                  <td>
                    {
                      forwardLog.map((item)=>(
                        item.fwType === 1 && <div className={styles.circle} key={item.fwResult}>
                          <span style={{ background: item.fwResult === 'SUCCESS' ? '#02B583' : '#F54238'}}></span>
                          {formatMessage(item.fwResult === 'SUCCESS' ? search_msg.event_success : search_msg.event_fail)}
                        </div>
                      ))
                ...
                <tr>
                  ...
                  <td colSpan={5}>
                    ...
                    {
                      (videoList.length || imageList.length) ?
